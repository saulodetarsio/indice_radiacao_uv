char msg[] ="SAULO DE TARSIO  ";  //Change the text here.
...
//Columns
int clockPin1 = 12; //Arduino pin connected to Clock Pin 11 of 74HC595
int latchPin1 = 10; //Arduino pin connected to Latch Pin 12 of 74HC595
int dataPin1 = 11;  //Arduino pin connected to Data Pin 14 of 74HC595

//Rows
int clockPin2 = 7; //Arduino pin connected to Clock Pin 11 of 74HC595
int latchPin2 = 5; //Arduino pin connected to Latch Pin 12 of 74HC595
int dataPin2 = 6;  //Arduino pin connected to Data Pin 14 of 74HC595
...
int count = 0;
void loop() {
Serial.println("Amnesia");
 //Linhas
 digitalWrite(latchPin2, LOW);//Hold latchPin LOW for transmitting data
 digitalWrite(latchPin1, LOW);//Hold latchPin LOW for transmitting data
 shiftOut(dataPin2, clockPin2, MSBFIRST, 0b00000000);   //Transmit data
 shiftOut(dataPin1, clockPin1, MSBFIRST, 0b10000000);   //Transmit data
 digitalWrite(latchPin2, HIGH);//Return the latch pin 2 high to signal chip
 digitalWrite(latchPin1, HIGH);//Hold latchPin LOW for transmitting data
...
//Colunas
 digitalWrite(latchPin1, LOW);//Return the latch pin 2 high to signal chip
 shiftOut(dataPin1, clockPin1, LSBFIRST, 0b11111111);
 digitalWrite(latchPin1, HIGH);//Return the latch pin 2 high to signal chip

 // digitalWrite(latchPin2, LOW);//Hold latchPin LOW for transmitting data
 //shiftOut(dataPin2, clockPin2, MSBFIRST, b3);   //Transmit data
 //digitalWrite(latchPin2, HIGH);//Return the latch pin 2 high to signal chip

// digitalWrite(latchPin1, LOW);//Return the latch pin 2 high to signal chip
 //shiftOut(dataPin1, clockPin1, MSBFIRST, b4);
// digitalWrite(latchPin1, HIGH);//Return the latch pin 2 high to signal chip
**/
// XProcess();
 count = count+1;
